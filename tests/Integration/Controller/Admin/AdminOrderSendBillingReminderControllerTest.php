...
class AdminOrderSendBillingReminderControllerTest extends WebTestCase
{
    ...
    {
        [$order] = $this->createManualBillingOrder();
    ...
    public function testSendPersistsAuditRow(): void
    {
        ...
        $this->assertSame('billing_reminder', $auditLog->payload['email_type']);
    }

    public function testNoAuditRowWithoutPendingPaymentRequest(): void
    {
        [$order] = $this->createManualBillingOrder(withPendingRequest: false);
        $this->entityManager->flush();
        $orderId = $order->id->toRfc4122();
    ...
    {
        return '/portal/admin/orders/'.$order->id->toRfc4122().'/send-billing-reminder';
    }

    /**
     * @return array{0: Order, 1: Contract}
     */
    private function createManualBillingOrder(bool $withPendingRequest = true): array
    {
        $now = $this->clock->now();
