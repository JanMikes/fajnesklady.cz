...
class AdminOrderSettleDebtControllerTest extends WebTestCase
{
    ...
    public function testSettleFullClearsDebtAndRecordsPayment(): void
    {
        ...
        $this->assertNull($reloaded->outstandingDebtAmount);

        $this->assertNotNull($this->findPaymentForContract($contractId));
        $this->assertNotNull($this->findAuditRow($contractId, 'debt_settled'));
    }

    public function testSettlePartialLeavesRemainder(): void
    {
        [$order, $contract] = $this->createTerminatedContractWithDebt(50000);
        $this->entityManager->flush();
        $orderId = $order->id->toRfc4122();
        ...
        $reloaded = $this->entityManager->find(Contract::class, Uuid::fromString($contractId));
        $this->assertInstanceOf(Contract::class, $reloaded);
        $this->assertTrue($reloaded->hasOutstandingDebt());
        $this->assertSame(30000, $reloaded->outstandingDebtAmount);
    }
    ...
    public function testRejectsWhenNoDebt(): void
    {
        [$order] = $this->createTerminatedContractWithDebt(0);
        $this->entityManager->flush();

        $this->loginAsAdmin();
        $this->client->request('POST', $this->url($order), ['amount' => '100']);

        $flashes = $this->client->getRequest()->getSession()->getFlashBag()->get('error');
        $this->assertNotEmpty($flashes);
    }

    public function testDetailPageRendersDebtActions(): void
    {
        // Exercises the canSettleDebt Twig branch on the order detail: the two
        // buttons + the settle <dialog> and the waive _danger_modal embed.
        ...
        $this->assertSelectorTextContains('body', 'Označit dluh jako uhrazený');
        $this->assertSelectorTextContains('body', 'Odepsat dluh');
    }
    ...
    {
        return '/portal/admin/orders/'.$order->id->toRfc4122().'/settle-debt';
    }
