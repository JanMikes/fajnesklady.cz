...
use App\Command\BulkGenerateStorageCodesCommand;
...
#[Route(
    '/portal/places/{placeId}/access-codes/bulk-generate',
    name: 'portal_place_access_codes_bulk_generate',
    methods: ['POST'],
)]
#[IsGranted('ROLE_LANDLORD')]
final class PlaceAccessCodesBulkGenerateController extends AbstractController
{
    ...
    public function __invoke(string $placeId): Response
    {
        ...
        try {
            $envelope = $this->commandBus->dispatch(new BulkGenerateStorageCodesCommand(placeId: $place->id));
            $count = (int) ($envelope->last(HandledStamp::class)?->getResult() ?? 0);

            $this->addFlash('success', sprintf('Doplněno %d kódů.', $count));
        } catch (HandlerFailedException $e) {
            $nested = $e->getPrevious();
            if ($nested instanceof StorageCodeRangeExhausted) {
