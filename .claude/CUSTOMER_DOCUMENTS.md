...
- **Persisted contract document parity**: the canonical signed PDF on disk
  (`$contract->documentPath`, generated by `ContractService::generateDocument`
  for portal/admin download) uses `$contract->createdAt` for the printed
  `CONTRACT_DATE` placeholder, whereas the e-mail attachments use
  `$order->createdAt` (via `renderBytesForOrder`). They differ only in
  this one date field. Unify by switching the persisted-file generator to
  the same order-based render so portal-downloaded contract === emailed
