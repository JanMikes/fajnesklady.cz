...
| 020 | Public order status permalink — signed `/objednavka/{id}/stav?_hash=…` (UriSigner) replacing `/dokonceno`; lifecycle-aware page (Aktivní/Čeká na platbu/Zrušeno/Pozastaveno/…) with state-specific CTAs, all order docs gated by signature, CTA wired into every customer-facing e-mail | ready | [020-public-order-status-permalink.md](020-public-order-status-permalink.md) |
| 021 | Customer never sees a lifetime total (always "Měsíční platba / měsíc" for recurring); admin & landlord see "Zaplaceno" + "K zaplacení celkem" (only for fixed-term recurring; absent for unlimited). Subsumes 013 | ready | [021-customer-price-display.md](021-customer-price-display.md) |
